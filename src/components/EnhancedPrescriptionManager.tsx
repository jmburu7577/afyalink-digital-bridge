...
import { Pill, Edit, Send, Plus, X, Copy } from 'lucide-react';
...
const EnhancedPrescriptionManager: React.FC<EnhancedPrescriptionManagerProps> = ({ patientId, appointmentId }) => {
  ...
  const handleMedicationSelect = (index: number, medicationData: any) => {
    setCurrentPrescription(prev => ({
      ...prev,
      medications: prev.medications.map((med, i) => 
        i === index ? { 
          ...
          frequency: medicationData.commonFrequencies?.[0] || '',
          duration: medicationData.commonDurations?.[0] || '',
          instructions: medicationData.commonInstructions?.[0] || ''
        } : med
      )
    }));
  };
  ...
  const duplicatePrescription = (prescription: any) => {
    // Parse existing prescription and create a duplicate
    ...
    setCurrentPrescription({
      diagnosis,
      medications: [
        { name: 'Amlodipine', dosage: '5mg', frequency: 'Once daily', duration: '30 days', instructions: 'Take after meals' }
      ],
    ...
    toast({
      ...
      description: "You can now modify and save the duplicated prescription."
    });
  };
  ...
  const sendToPharmacy = async (prescription: any) => {
    ...
    setTimeout(() => {
      toast({
        ...
        description: "Prescription has been sent to the pharmacy for processing.",
      });
    }, 2000);
  };
          ...
          <div key={index} className="border rounded-lg p-4 mb-4 space-y-3">
            ...
            <div className="space-y-3">
              <div>
                ...
                <MedicationDatabase
                  onSelectMedication={(medicationData) => handleMedicationSelect(index, medicationData)}
                />
                <Input
      ...
      <Card className="bg-white/10 backdrop-blur-lg border-white/20 shadow-xl">
        ...
        <CardContent>
          <div className="space-y-4">
            {/* Sample prescription with enhanced actions */}
